...
class InvestController extends Controller
{
    ...
    public function log()
    {
        ...
        $investments               = Invest::where('user_id', $user->id)->sum('amount');
        $completedInvests      = Invest::where('user_id', $user->id)->where('status', 0)->sum('amount');
        $runningInvests        = Invest::where('user_id', $user->id)->where('status', 1)->sum('amount');
        $interests             = Transaction::where('remark', 'interest')->where('user_id', $user->id)->sum('amount');
        return view(activeTemplate() . 'user.invests', compact('pageTitle', 'invests','investments','completedInvests','runningInvests','interests'));
    }
